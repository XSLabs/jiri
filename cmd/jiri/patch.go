...
func patchProject(jirix *jiri.X, local project.Project, ref, branch, remote string) (bool, error) {
		...
		if branchExists {
			if patchDeleteFlag {
				...
				if currentBranch == branch {
					if err := scm.CheckoutBranch("remotes/origin/"+remote, (local.GitSubmodules && jirix.EnableSubmodules), gitutil.DetachOpt(true)); err != nil {
						return false, err
					}
				}
	...
	if err := scm.CheckoutBranch(branchBase, (local.GitSubmodules && jirix.EnableSubmodules)); err != nil {
		return false, err
	}
	if cherryPickFlag {
		if err := scm.CherryPick("FETCH_HEAD"); err != nil {
			...
			// checkout last ref
			if err := scm.CheckoutBranch(lastRef, (local.GitSubmodules && jirix.EnableSubmodules)); err != nil {
